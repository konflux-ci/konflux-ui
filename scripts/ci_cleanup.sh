...
clean_konflux() {
    ...
    if [[ "$CURRENT_NAMESPACE" == "$KONFLUX_NAMESPACE" ]]; then
        ...
        echo "Access denied: The current user is authorized for $KONFLUX_NAMESPACE but does not have permission to access $CURRENT_NAMESPACE."
        exit 1
    fi
    ...
    if ! resources_json=$(oc get "$resources_type" -n "$KONFLUX_NAMESPACE" -o json); then
        echo "Error: Failed to fetch $resources_type from namespace $KONFLUX_NAMESPACE"
        exit 1
    fi
