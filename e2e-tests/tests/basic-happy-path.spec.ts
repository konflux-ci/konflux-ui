...
describe('Basic Happy Path', () => {
  ...
  describe('Explore Pipeline runs Tab', () => {
    ...
    it('Merge the auto-generated PR, and verify the event status on modal', () => {
      ...
      // we should add some check for the pop up here

      APIHelper.getMergedPRCount(repoOwner, repoName).then((mergedCountBefore: number) => {
        cy.log(`Merged PR count before merging a PR: ${mergedCountBefore}`);
        ...
        // Wait for 2 minutes for the PR to be merged in GitHub
        const waitForMerge = (): Cypress.Chainable<void> => {
          return APIHelper.getMergedPRCount(repoOwner, repoName).then(
            (mergedCountAfter: number) => {
              cy.log(`Merged PR count after merging a PR: ${mergedCountAfter}`);
              if (mergedCountAfter > mergedCountBefore) {
                return;
              }
              return cy.wait(2000).then(waitForMerge);
            },
          );
        };
      ...
      componentPage.clickCustomButton();

      // Wait for UI to update the status of the merged PR
