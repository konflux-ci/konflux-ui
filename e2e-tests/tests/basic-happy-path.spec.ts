...
describe('Basic Happy Path', () => {
  ...
  after(function () {
    ...
    if (allTestsSucceeded || Cypress.env('REMOVE_APP_ON_FAIL')) {
      // Use UI to remove the application to test the delete flow
      Common.navigateTo(NavItem.applications);

      // Wait for the application row to be visible before attempting deletion
      cy.get(`[data-id="${applicationName}"]`, { timeout: 30000 })
        .should('exist')
        .then(() => {
          ...
          cy.get(`[data-id="${applicationName}"]`, { timeout: 60000 })
            .should('not.exist')
            .then(() => {
