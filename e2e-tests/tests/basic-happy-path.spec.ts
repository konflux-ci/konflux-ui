...
describe('Basic Happy Path', () => {
  ...
  describe('Explore Pipeline runs Tab', () => {
    ...
    it('Merge the auto-generated PR, and verify the event status on modal', () => {
      ...
      // The build service may set build-status message to "done" before the
      // PR is merged, causing the component to show "Custom" (PACState.ready)
      // instead of "Merge pull request" (PACState.pending). Handle both.
      componentPage.clickMergePullRequestIfPending().then((modalOpened) => {
        APIHelper.mergePR(
          repoOwner,
          repoName,
          1,
