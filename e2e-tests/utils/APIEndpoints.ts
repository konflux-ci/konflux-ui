export const hacAPIEndpoints = {
  applications: (applicationName: string) =>
    `/api/k8s/apis/appstudio.redhat.com/v1alpha1/namespaces/${Cypress.expose(
      'HAC_NAMESPACE',
  ...
  pipelinerunsFilter: (applicationName: string, label: string) =>
    `/api/k8s/apis/tekton.dev/v1beta1/namespaces/${Cypress.expose(
      'HAC_NAMESPACE',
    )}/pipelineruns?labelSelector=appstudio.openshift.io/application=${applicationName},${label}&limit=250`,

  secrets: (secretName: string) =>
    `/api/k8s/api/v1/namespaces/${Cypress.expose('HAC_NAMESPACE')}/secrets/${secretName}`,

  resources: (resourceType: string) =>
    `/api/k8s/apis/appstudio.redhat.com/v1beta1/namespaces/${Cypress.expose(
      'HAC_NAMESPACE',
    )}/${resourceType}s`,
};
