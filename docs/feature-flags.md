...
_There is **no** backend call. Everything happens in the browser._
...
- `?ff=flagA,flagB` → turn _on_ multiple flags quickly.
...
3. **Commit** your changes; reviewers can now enable the flag locally or via URL to test.
...
2. Once verified, Remove obsolete branches, then delete the flag entry.
