...
App (main.tsx)                                 # Renders after auth
  ├── useAnalyticsInitialization()             # Starts analytics lifecycle
  ...
  │     ├── analyticsService.identify()         # Store pseudonymous userId for events
...
| `src/analytics/useAnalyticsInitialization.ts` | Initializes common properties, pseudonymous user event properties, login tracking, and session-start tracking |
...
After authentication, `useAnalyticsInitialization()` derives a stable, cluster-scoped pseudonymous `userId` by SHA-256 hashing `preferredUsername:clusterId`. The per-tab `sessionId` is Segment's `anonymousId` transport identity. `AnalyticsService.identify()` stores the obfuscated `userId`, and `AnalyticsService.track()` adds it only to each event's `userId` property. The raw username and `clusterId` must not appear in event payloads.
