...
export class WebSocketFactory {
  ...
  private reconnect(): void {
    ...
    const attempt = () => {
      ...
      console.info(`attempting reconnect in ${duration / 1000} seconds...`);
    };
  ...
  private connect(): void {
    ...
    const subProtocols = applyConfigSubProtocols(this.options);
    try {
    ...
    this.ws.onclose = (evt) => {
      console.info(`websocket closed: ${this.id}`, evt);
      this.state = WebSocketState.CLOSED;
      this.triggerEvent('close', evt);
      this.reconnect();
    };
    this.ws.onerror = (evt) => {
      console.info(`websocket error: ${this.id}`);
    ...
    this.ws.onmessage = (evt) => {
      let msg;
      try {
        msg = this.options.jsonParse ? JSON.parse(evt.data as string) : evt.data;
      } catch (e) {
        console.warn(
  ...
  private invokeHandlers(type: EventHandlerTypes, data?: unknown): void {
    ...
    handlers?.forEach((h) => {
      try {
        ...
        console.error('Web socket handling failed:', e);
      }
    });
  }
  ...
  destroy(eventData?: unknown): void {
    console.info(`destroying websocket: ${this.id}`);
    if (this.state === WebSocketState.DESTROYED) {
      return;
    }
    ...
    try {
      ...
      console.error('Error while close web socket socket', e);
    }
    ...
    try {
      ...
      console.error('Error while trigger destroy event for web socket', e);
    }
