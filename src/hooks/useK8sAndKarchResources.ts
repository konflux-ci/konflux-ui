...
export interface K8sAndKarchResourcesResult<T extends K8sResourceCommon> {
  ...
  // Determines if a resource came from cluster or archive
  getSource: (resource: T) => ResourceSource | undefined;
}
...
export function useK8sAndKarchResources<T extends K8sResourceCommon>(
  ...
  // deduplicated data
  const [combinedData, sourceMap] = React.useMemo(() => {
    ...
    const combined: T[] = [];
    const dataSourceMap = new Map<string, ResourceSource>();

    if (shouldIncludeCluster) {
      clusterData.forEach((item) => {
        const id = getResourceId(item);
        combined.push(item);
        dataSourceMap.set(id, ResourceSource.Cluster);
      });
    }
    ...
    if (shouldIncludeArchive) {
      archiveData.forEach((item) => {
        ...
        if (!dataSourceMap.has(id)) {
          combined.push(item);
          dataSourceMap.set(id, ResourceSource.Archive);
        }
      });
    }

    return [combined, dataSourceMap];
  }, [
    clusterQuery.data,
  ...
  return React.useMemo(() => {
    ...
    const getSource = (resource: T): ResourceSource | undefined => {
      return sourceMap.get(getResourceId(resource));
    };
