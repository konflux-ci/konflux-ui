...
describe('useWorkspaceConformaViolations', () => {
  ...
  it('returns settling=true with partial data while logs are still loading', () => {
    const comp = createComponent('comp-a', 'app-a');
    mockUseAllComponents.mockReturnValue([[comp], true, undefined]);
    mockUseComponentsConformaResults.mockReturnValue({
      ...DEFAULT_SHARED,
      logsSettled: false,
      conformaByComponent: new Map([['comp-a', makeConformaData(2)]]),
    });
  ...
  it('surfaces fatal error even while settling', () => {
    const fatalError = new Error('taskruns failed');
    mockUseAllComponents.mockReturnValue([[], true, undefined]);
    mockUseComponentsConformaResults.mockReturnValue({
      ...DEFAULT_SHARED,
      logsSettled: false,
      fillInSettled: false,
      taskRunsError: fatalError,
    });
    ...
    expect(result.current.loaded).toBe(true);
    expect(result.current.settling).toBe(true);
    expect(result.current.error).toBe(fatalError);
  });

  it('returns settling=false when all queries are settled', () => {
    mockUseAllComponents.mockReturnValue([[], true, undefined]);
