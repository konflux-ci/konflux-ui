...
export const SecretTypeSubForm: React.FC<React.PropsWithChildren<unknown>> = () => {
  ...
  const selectedForm = React.useMemo(() => {
    ...
    if (form?.key === 'source') {
      form.component = <SourceSecretForm onAuthTypeChange={(type) => setCurrentAuthType(type)} />;
    }
    return form;
  }, [currentType]);
  ...
  const shouldShowSecretLinkOptions =
    currentType === secretTypes.find((t) => t.key === 'image-pull').label ||
    currentAuthType === SourceSecretType.basic;
