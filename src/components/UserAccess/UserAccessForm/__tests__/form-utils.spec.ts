...
describe('createRBs', () => {
  it('should create RBs for all usernames', async () => {
    ...
    const expectedResult = ['user1', 'user2', 'user3'].map((username) => ({
      ...mockRoleBinding,
      metadata: {
...
describe('deleteRB', () => {
  ...
  it('should not call K8sQueryDeleteResource with dryRun query parameter when dryRun is not passed', async () => {
    k8sDeleteMock.mockResolvedValueOnce(undefined);
