...
import ComponentList, { COMPONENTS_LIST_GS_LOCAL_STORAGE_KEY } from '../ComponentList';
...
const useLatestPushBuildPipelinesMock = jest.requireMock('~/hooks/useLatestPushBuildPipelines')
  .useLatestPushBuildPipelines as jest.Mock;
...
describe('ComponentList', () => {
  ...
  it('exports COMPONENTS_LIST_GS_LOCAL_STORAGE_KEY', () => {
    expect(COMPONENTS_LIST_GS_LOCAL_STORAGE_KEY).toBe('components-list-getting-started-modal');
  });
  ...
  it('calls useLatestPushBuildPipelines with namespace and component names', () => {
    const components = [createMockComponent('c1')];
    useAllComponentsMock.mockReturnValue([components, true, undefined]);
    renderComponentList();
  ...
  it('renders table when pipeline runs are not yet loaded', () => {
