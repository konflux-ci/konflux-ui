...
/** Kubernetes namespace names are DNS-1123 labels (max 63 characters). */
...
export const isSafeNamespace = (value: string | undefined): value is string =>
  !!value && DNS_1123_LABEL.test(value);
...
export const buildOcLoginCommand = (apiServerUrl: string, namespace?: string): string => {
  ...
  const namespaceFlag = isSafeNamespace(namespace) ? ` -n ${namespace}` : '';
  if (!origin) {
...
export const buildKubernetesLoginCommand = (namespace?: string, context?: string): string => {
  ...
  return steps.length > 0 ? steps.join(' && ') : 'kubectl config get-contexts';
};
