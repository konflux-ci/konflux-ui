...
describe('buildKubernetesLoginCommand', () => {
  ...
  it('falls back to listing contexts when neither context nor namespace is provided', () => {
    expect(buildKubernetesLoginCommand()).toBe(KUBECTL_LIST_CONTEXTS_COMMAND);
    expect(buildKubernetesLoginCommand('')).toBe(KUBECTL_LIST_CONTEXTS_COMMAND);
  });

  it('omits an unsafe context and keeps the namespace', () => {
    expect(buildKubernetesLoginCommand('my-tenant', 'ctx$(id)')).toBe(
      'kubectl config set-context --current --namespace=my-tenant',
    );
  });
});
...
describe('buildLoginCommand', () => {
  ...
  it('builds a cluster-only command when the user is not in a tenant', () => {
    expect(buildLoginCommand('openshift', 'https://api.example.com:6443', '')).toBe(
      'oc login https://api.example.com:6443 --web',
    );
    expect(buildLoginCommand('kubernetes', 'https://127.0.0.1:37689', '', 'kind-konflux')).toBe(
      'kubectl config use-context kind-konflux',
    );
  });
});
