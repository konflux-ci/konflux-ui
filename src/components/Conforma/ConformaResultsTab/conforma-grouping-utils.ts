...
export const collapseArchDuplicates = (rows: ConformaResultRow[]): ConformaResultRow[] => {
  ...
  for (const row of rows) {
    const key = `${row.title}\0${row.msg ?? ''}\0${row.component}\0${row.status}`;
    const existing = map.get(key);
    if (existing) {
      if (row.image && !existing.images.includes(row.image)) {
        existing.images = [...existing.images, row.image];
      }
    } else {
      map.set(key, { ...row, images: row.image ? [row.image] : [] });
    }
  }
