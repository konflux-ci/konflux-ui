...
describe('ConformaGroupedTable', () => {
  ...
  it('shows truncated image when row has a single image and no multi-image array', () => {
    const groupsWithSingleImage: GroupedConformaRow[] = [
      {
        ...
        warnings: 0,
        successes: 0,
        rows: [createRow({ image: 'quay.io/test/img@sha256:only', images: undefined })],
      },
    ];
    ...
    const groupsWithSingleCollapsedImage: GroupedConformaRow[] = [
      {
        ...
        rows: [
          createRow({
            ...
            image: 'quay.io/test/img@sha256:only',
          }),
        ],
      },
    ];
