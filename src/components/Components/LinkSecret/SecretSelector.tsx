...
export const SecretSelector: React.FC<React.PropsWithChildren<SecretSelectorProps>> = ({
  ...
  const secretsErrorState = useErrorState(secretsError, secretsLoaded, 'secrets');
  const [previouslyLinked] = useLinkedSecrets(namespace, componentName);
  const componentErrorState = useErrorState(compError, compLoaded, 'component');
  ...
  if (compError) {
    return componentErrorState;
  }

  if (secretsError) {
    return secretsErrorState;
  }
