...
describe('useRegistryLoginUrl', () => {
  ...
  it('should return correct URL when domain is configured', () => {
    const mockDomain = 'apps.kflux-ocp-p01.7ayg.p1.openshiftapps.com';
    mockUseKonfluxPublicInfo.mockReturnValue([{ domain: mockDomain }, true, null]);

    const { result } = renderHook(() => useRegistryLoginUrl());

    const expectedUrl = `https://oauth-openshift.${mockDomain}/oauth/token/request`;
    expect(result.current).toEqual([expectedUrl, true, null]);
  });

  it('should construct URL correctly with different domains', () => {
    const mockDomain = 'custom.domain.example.com';
    mockUseKonfluxPublicInfo.mockReturnValue([{ domain: mockDomain }, true, null]);
