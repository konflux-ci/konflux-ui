...
import { mockUseNamespaceHook, routerRenderer } from '~/unit-test-utils';
...
describe('useComponentGroupBreadcrumbs', () => {
  ...
  it('should only return the groups crumb when no group name is passed', () => {
    const { result } = renderHook(() => useComponentGroupBreadcrumbs(undefined));
  ...
  it('should return a link to the groups list plus the group crumb when group name is passed', () => {
    const { result } = renderHook(() => useComponentGroupBreadcrumbs('test-group'));
