import { LIGHTSPEED_API_BASE, lightspeedVersionedPath } from '~/lightspeed/lightspeedConfig';
...
/**
 * Rename is not exposed by `@redhat-cloud-services/lightspeed-client` yet.
 * Persist the display name via Lightspeed `topic_summary`.
 */
export const updateConversationTopicSummary = async (
  ...
  if (!response.ok) {
    ...
    throw new Error(getUserFacingErrorMessage(response.status));
  }
};
