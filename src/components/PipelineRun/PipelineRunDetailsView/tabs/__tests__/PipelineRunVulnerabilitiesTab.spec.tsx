...
import { TektonResourceLabel, type PipelineRunKind, type TaskRunKind } from '~/types';
...
const mockRunningPipelineRun: PipelineRunKind = {
  ...
  status: {
    ...mockCompletedPipelineRun.status,
...
describe('PipelineRunVulnerabilitiesTab', () => {
  ...
  it('should show a pending state when the pipeline is still running without a scan task run', () => {
    usePipelineRunMock.mockReturnValue([mockRunningPipelineRun, true, null]);
    useTaskRunsMock.mockReturnValue([[], true, null]);

    renderWithQueryClientAndRouter(<PipelineRunVulnerabilitiesTab />);

    screen.getByText('Vulnerability scan pending');
    screen.getByText('The roxctl-scan task run has not started yet.');
  });
