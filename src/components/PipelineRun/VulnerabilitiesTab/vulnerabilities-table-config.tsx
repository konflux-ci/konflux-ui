...
export const vulnerabilitiesFilterConfigs = defineFilters<unknown>()([
  {
    type: 'search',
    param: 'cve',
    label: 'Search by CVE ID',
    placeholder: 'Search by CVE ID...',
    filterFn: (item, value) => isRoxctlCveTableRow(item) && textMatch(item.cve, value),
  },
  {
    type: 'multiSelect',
    param: 'package',
    ...
    filterFn: (item, values) =>
      isRoxctlCveTableRow(item) && item.components.some((c) => values.includes(c.name)),
  },
  {
    type: 'multiSelect',
    param: 'severity',
  ...
  },
  {
    ...
    label: 'Image',
    filterFn: (item, values) =>
      isRoxctlCveTableRow(item) && values.includes(imageColumnValue(item) ?? ''),
  },
] as const);
