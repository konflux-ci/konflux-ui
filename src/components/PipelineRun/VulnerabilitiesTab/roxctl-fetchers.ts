...
/**
 * Fetches the roxctl CVE report from the live cluster by reading the
 * `step-proccess-output` container log directly.
 */
export function fetchCveReportFromCluster(
  namespace: string,
  taskRun: TaskRunKind,
): Promise<RoxctlCveReportResolution> {
  return fetchCveReportFromPod(namespace, taskRun);
}
...
export async function resolveRoxctlCveReport(
  ...
  try {
    return await fetchCveReportFromCluster(namespace, taskRun);
  } catch (clusterError) {
