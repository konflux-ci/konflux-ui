...
describe('VulnerabilitiesTable', () => {
  ...
  it('renders severity labels with the correct text', () => {
    const noneSeverityRow: RoxctlCveTableRow = {
      ...mockData[0],
      cve: 'CVE-2025-0002',
      severity: 'NONE',
    };

    renderTable([...mockData, noneSeverityRow]);
  ...
  it('filters by package and renders the matching component details', () => {
    renderTable(mockData, '?package=%5B%22micromatch%22%5D');
    const table = within(screen.getByRole('grid', { name: 'Vulnerabilities' }));

    expect(table.getByText('CVE-2024-4068')).toBeInTheDocument();
    expect(table.getByText('micromatch')).toBeInTheDocument();
    expect(table.getByText('4.0.5')).toBeInTheDocument();
    expect(table.getByText('NODEJS')).toBeInTheDocument();
    expect(table.queryByText('braces (+1 more)')).not.toBeInTheDocument();
    expect(table.queryByText('3.0.2')).not.toBeInTheDocument();
  });

  it('filters by image search param', () => {
    const otherImageRow: RoxctlCveTableRow = {
      ...mockData[0],
      cve: 'CVE-2025-0001',
      imageFullName: 'quay.io/other:latest',
    };

    renderTable([...mockData, otherImageRow], '?image=%5B%22quay.io%2Fother%3Alatest%22%5D');

    expect(screen.getByText('CVE-2025-0001')).toBeInTheDocument();
    expect(screen.queryByText('CVE-2024-21626')).not.toBeInTheDocument();
  });

  it('filters by None severity search param', () => {
    const noneSeverityRow: RoxctlCveTableRow = {
      ...mockData[0],
      cve: 'CVE-2025-0002',
      severity: 'NONE',
    ...
    renderTable([...mockData, noneSeverityRow], '?severity=%5B%22none%22%5D');

    expect(screen.getByText('CVE-2025-0002')).toBeInTheDocument();
    expect(screen.queryByText('CVE-2024-21626')).not.toBeInTheDocument();
  });

  it('renders a CVE without a link as plain text', () => {
    const rowWithoutLink: RoxctlCveTableRow = {
      ...mockData[0],
      cve: 'CVE-2025-0003',
      link: undefined,
    };

    renderTable([rowWithoutLink]);

    const cveText = screen.getByText('CVE-2025-0003');
    expect(cveText.closest('a')).toBeNull();
  });
