...
describe('extractCveReportFromTaskRunLogs', () => {
  describe('flat array format (production)', () => {
    ...
    it('extracts JSON array when it is the last step (no trailing step boundary)', () => {
      const logs = ['step-proccess-output :-', JSON.stringify(sampleFlatEntries)].join('\n');
  ...
  describe('nested object format (legacy)', () => {
    ...
    it('extracts JSON when it is the last step (no trailing step boundary)', () => {
      const logs = ['step-proccess-output :-', JSON.stringify(sampleReport)].join('\n');
...
describe('flattenCveData', () => {
  it('normalizes both supported roxctl report formats', () => {
    expect(flattenCveData(sampleFlatEntries)[0]).toMatchObject({
      cve: 'CVE-2021-20197',
      componentName: 'binutils-gold',
    });
    expect(flattenCveData(sampleReport)[0]).toMatchObject({
      cve: 'CVE-2024-0001',
      componentName: 'openssl',
      imageFullName: 'quay.io/redhat/test-image:latest',
    });
...
describe('toRows', () => {
  it('flattens a flat report format', () => {
  ...
  it('flattens a nested report format', () => {
  ...
  it('pairs each report with its platform for multi-arch builds', () => {
    ...
    const rows = toRows([sampleFlatEntries, archEntries], ['linux/amd64', 'linux/arm64']);
...
describe('imageColumnValue', () => {
  ...
  it('falls back to imageFullName when imagePlatform is absent', () => {
    expect(imageColumnValue({ imageFullName: 'quay.io/test:latest' })).toBe('quay.io/test:latest');
  });
...
describe('groupRowsByCve', () => {
  ...
  it('handles undefined componentSource', () => {
    const rows = flattenCveReport({
      ...
      scan: {
        components: [
          { name: 'lib', version: '1.0', vulns: [{ cve: 'CVE-TEST', severity: 'LOW' }] },
        ],
      },
    });
    const grouped = groupRowsByCve(rows);
  ...
  it('preserves insertion order of CVEs', () => {
    const rows = [
      ...flattenFlatCveEntries([
        { cve: 'CVE-B', severity: 'HIGH', components: [{ component: 'pkg-b', version: '1.0' }] },
      ]),
      ...flattenFlatCveEntries([
        { cve: 'CVE-A', severity: 'LOW', components: [{ component: 'pkg-a', version: '1.0' }] },
      ]),
      ...flattenFlatCveEntries([
        { cve: 'CVE-C', severity: 'MEDIUM', components: [{ component: 'pkg-c', version: '1.0' }] },
      ]),
    ];
        ...
        components: [
          {
            ...
            version: '3.0.5',
            source: 'OS',
            vulns: [{ cve: 'CVE-2024-FIELD', severity: 'HIGH' }],
          },
        ],
      },
    });
