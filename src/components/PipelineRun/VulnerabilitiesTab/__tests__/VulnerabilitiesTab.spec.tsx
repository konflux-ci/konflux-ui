...
describe('VulnerabilitiesTab', () => {
  ...
  it('should show an error state when the TaskRun fetch fails', () => {
    useTaskRunMock.mockReturnValue([null, true, new Error('Failed to fetch task run')]);
    useRoxctlCveReportMock.mockReturnValue({ data: [], isLoading: false, error: null });

    renderWithQueryClientAndRouter(<VulnerabilitiesTab />);

    screen.getByText('Unable to load task run');
  });
  ...
  it('should show the shared generic error message for non-Error objects', () => {
    useTaskRunMock.mockReturnValue([mockTaskRun, true]);
    useRoxctlCveReportMock.mockReturnValue({
      data: [],
      isLoading: false,
