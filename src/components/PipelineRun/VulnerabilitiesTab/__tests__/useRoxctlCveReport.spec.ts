...
const mockTaskRunWithoutPodName: TaskRunKind = {
  ...mockTaskRun,
  status: {
    conditions: [],
  },
...
describe('useRoxctlCveReport', () => {
  ...
  it('should refetch when the TaskRun status gains a pod name', async () => {
    ...
    const { result, rerender } = renderHook(
      ...
      {
        initialProps: { taskRun: mockTaskRunWithoutPodName },
        wrapper,
      },
    );
    ...
    await waitFor(() => {
      expect(mockResolveRoxctlCveReport).toHaveBeenCalledTimes(2);
    });
  });
