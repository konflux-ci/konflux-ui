...
const ReleasesListView: React.FC = () => {
  ...
  const filteredReleases = React.useMemo(() => {
    ...
    const opts = { fuzzy: true } as const;
    switch (filterType) {
      case FilterTypes.name:
        return releases.filter((r) => textMatch(r.metadata.name, searchFilter, opts));
      case FilterTypes.releasePlan:
        return releases.filter((r) => textMatch(r.spec.releasePlan, searchFilter, opts));
      case FilterTypes.releaseSnapshot:
        return releases.filter((r) => textMatch(r.spec.snapshot, searchFilter, opts));
      default:
        return releases;
    }
