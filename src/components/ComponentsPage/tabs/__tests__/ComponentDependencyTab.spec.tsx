...
describe('ComponentsPage ComponentDependencyTab', () => {
  mockUseNamespaceHook('test-ns');

  beforeEach(() => {
    useParamsMock.mockReturnValue({ componentName: 'standalone-component' });
  ...
  it('passes only the standalone component scope to the run list', () => {
    renderWithQueryClientAndRouter(<ComponentDependencyTab />);

    expect(useComponentMock).toHaveBeenCalledWith('test-ns', 'standalone-component', true);
    expect(screen.getByTestId('dependency-runs-props')).toHaveTextContent(
      JSON.stringify({
        componentNames: ['standalone-component'],
