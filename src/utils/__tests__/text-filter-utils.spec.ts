...
describe('textMatch', () => {
  ...
  describe('non-string value handling', () => {
    ...
    it('should coerce a numeric search term to a string', () => {
      expect(textMatch('version-42', 42 as unknown as string)).toBe(true);
    });
    ...
    it('should handle null search term gracefully', () => {
      expect(textMatch('anything', null as unknown as string)).toBe(true);
    });

    it('should handle undefined search term gracefully', () => {
      expect(textMatch('anything', undefined as unknown as string)).toBe(true);
    });
...
describe('filterByText', () => {
  ...
  it('should return the original array reference for null/undefined query', () => {
    expect(filterByText(items, null, (item) => item.name)).toBe(items);
  ...
  it('should support accessor returning a number', () => {
    const result = filterByText(items, '2', (item) => item.id);
    expect(result).toEqual([{ name: 'Beta', id: 2 }]);
  });
