...
describe('Prism Log Language', () => {
  ...
  describe('Complex Log Lines', () => {
    ...
    it('should tokenize error log with context', () => {
      const logLine = 'ERROR: Database connection failed host=localhost port=5432 time=10:30:45';
      const tokens = Prism.tokenize(logLine, Prism.languages.log);

      const errorLevel = tokens.find((t) => typeof t !== 'string' && t.type === 'log-level-error');
      const kvPairs = tokens.filter((t) => typeof t !== 'string' && t.type === 'key-value');

      expect(errorLevel).toBeDefined();
      expect(kvPairs.length).toBeGreaterThanOrEqual(2);
    });

    it('should tokenize test result log', () => {
      const logLine = '[10:30:45] Test suite PASSED duration=2.5s tests=10';
      const tokens = Prism.tokenize(logLine, Prism.languages.log);

      const timestamp = tokens.find((t) => typeof t !== 'string' && t.type === 'timestamp');
      const result = tokens.find((t) => typeof t !== 'string' && t.type === 'result');
      const kvPairs = tokens.filter((t) => typeof t !== 'string' && t.type === 'key-value');
