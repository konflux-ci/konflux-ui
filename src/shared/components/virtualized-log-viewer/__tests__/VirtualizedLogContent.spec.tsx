...
Another short line`;
  ...
  describe('Per-step fetch errors', () => {
    const sectionsWithError: LogSection[] = [
      { containerName: 'VALIDATE', data: '', isCompleted: true, error: 'Network error' },
      { containerName: 'REPORT-JSON', data: 'report generated', isCompleted: true },
    ];
    ...
    it('should show an error indicator only on the section that failed to fetch', () => {
      renderWithQueryClientAndRouter(
        <VirtualizedLogContent {...defaultProps} sections={sectionsWithError} />,
      );

      expect(
        screen.getByLabelText('Failed to fetch logs for this step: Network error'),
      ).toBeInTheDocument();
      // The successful section has no error indicator.
      expect(screen.queryByTestId('fold-header-error-REPORT-JSON')).not.toBeInTheDocument();
    });

    it('should still show the successful section while the other step failed', () => {
      renderWithQueryClientAndRouter(
        <VirtualizedLogContent {...defaultProps} sections={sectionsWithError} />,
      );

      // REPORT-JSON completed without error, so it's folded by default like any other
      // completed step; its logs are still fully available on expand.
      expect(screen.getByTestId('fold-header-REPORT-JSON')).toBeInTheDocument();
      expect(screen.queryByText('report generated')).not.toBeInTheDocument();

      fireEvent.click(screen.getByTestId('fold-header-REPORT-JSON'));

      expect(screen.getByText('report generated')).toBeInTheDocument();
    });

    it('should keep a failed section expanded by default even though it is marked completed', () => {
      renderWithQueryClientAndRouter(
        <VirtualizedLogContent {...defaultProps} sections={sectionsWithError} />,
      );

      // Without the error, a completed section would be auto-folded; the error keeps it
      // expanded so the failure isn't hidden behind a "N lines hidden" indicator.
      expect(screen.getByTestId('fold-header-VALIDATE')).toHaveAttribute('aria-expanded', 'true');
    });

    it('should show the error message in the expanded section body', () => {
      renderWithQueryClientAndRouter(
        <VirtualizedLogContent {...defaultProps} sections={sectionsWithError} />,
      );

      expect(screen.getByTestId('section-error-line')).toHaveTextContent(
        'Failed to fetch logs for this step: Network error',
      );
    });

    it('should let the failed section still be folded/unfolded manually', () => {
      renderWithQueryClientAndRouter(
        <VirtualizedLogContent {...defaultProps} sections={sectionsWithError} />,
      );

      fireEvent.click(screen.getByTestId('fold-header-VALIDATE'));

      expect(screen.getByTestId('fold-header-VALIDATE')).toHaveAttribute(
        'aria-expanded',
        'false',
      );
      // The header icon remains; the body error line is hidden while folded.
      expect(
        screen.getByLabelText('Failed to fetch logs for this step: Network error'),
      ).toBeInTheDocument();
      expect(screen.queryByTestId('section-error-line')).not.toBeInTheDocument();
    });
  });
});
