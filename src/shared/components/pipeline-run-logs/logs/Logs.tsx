...
export const processLogs = (logSources: LogSources, containers: ContainerSpec[]): string => {
  ...
  const BOLD = '\x1b[1m';
  const WHITE_BACKGROUND_COLOR = '\x1b[47m';
  ...
  for (const container of containers) {
    ...
    if (logSources[containerName]) {
      allLogs += `\n\n${BOLD}${WHITE_BACKGROUND_COLOR}${BLACK_TEXT_COLOR}${containerName.toUpperCase()}${RESET}\n`;
...
const Logs: React.FC<LogsProps> = ({
  ...
  React.useEffect(() => {
    setCurrentLogsGetter(() => formattedLogs);
  }, [setCurrentLogsGetter, formattedLogs]);
  ...
  return (
    <div className="logs" data-test="logs-container">
      ...
      <LogViewer data={formattedLogs} autoScroll={autoScroll} onScroll={onScroll} />
    </div>
  );
};
