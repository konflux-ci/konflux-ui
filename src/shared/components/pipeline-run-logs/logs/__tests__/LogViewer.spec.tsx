...
describe('LogViewer Integration Tests', () => {
  ...
  describe('Resume stream button', () => {
    it('should handle showResumeStreamButton prop without crashing', () => {
      // The resume button appears conditionally based on scroll state
      // This test ensures the prop is handled correctly
      expect(() => {
        render(<LogViewer {...defaultProps} allowAutoScroll={true} />);
      }).not.toThrow();
    });

    it('should handle onResumeStream callback prop', () => {
      // The component should accept the onResumeStream prop
      expect(() => {
        render(<LogViewer {...defaultProps} allowAutoScroll={true} />);
      }).not.toThrow();
    });
