...
class PipelineRunLogs extends React.Component<PipelineRunLogsProps, PipelineRunLogsState> {
  ...
  getTaskRunLabel = (taskRun: TaskRunKind, taskRunName: string) => {
    ...
    if (taskRunName?.startsWith(`${pipelineRunName}-`)) {
  ...
  getTaskRunName = (taskRunName: string) => {
    ...
    return this.getTaskRunLabel(taskRun, taskRunName);
  };
  ...
  getSortedTaskRun = (tRuns: TaskRunKind[], tasks: PipelineTask[]): TaskRunKind[] => {
    ...
    return (
      taskRuns?.sort((a, b) => {
        const cTaskLabel = a?.metadata?.labels?.[TektonResourceLabel.pipelineTask];
        const dTaskLabel = b?.metadata?.labels?.[TektonResourceLabel.pipelineTask];
        const cTaskIndex = pipelineTaskNames?.indexOf(cTaskLabel) ?? -1;
        const dTaskIndex = pipelineTaskNames?.indexOf(dTaskLabel) ?? -1;

        // sort by pipeline task order
        const taskOrderDiff = cTaskIndex - dTaskIndex;
        if (taskOrderDiff !== 0) {
