...
describe('LoadingSkeleton', () => {
  it('renders the given count of skeleton items', () => {
    ...
    const items = container.querySelectorAll('.pf-v6-c-skeleton');
    expect(items.length).toBe(3);
  });

  it('applies widths for each skeleton item', () => {
    const { container } = render(<LoadingSkeleton count={2} widths={['50%', '25%']} />);
    const items = Array.from(container.querySelectorAll('.pf-v6-c-skeleton'));
    expect(items[0].getAttribute('style')).toContain('--pf-v6-c-skeleton--Width: 50%');
    expect(items[1].getAttribute('style')).toContain('--pf-v6-c-skeleton--Width: 25%');
  });
