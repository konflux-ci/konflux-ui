...
describe('ImageUrlDisplay', () => {
  ...
  it('should show external link for public images', () => {
    ...
    expectExternalLink(testImageUrl, `https://${testImageUrl}`);
  });
  ...
  it('should not add https:// prefix for URLs that already have protocol', () => {
    ...
    expectExternalLink(urlWithProtocol, urlWithProtocol);
  });
  ...
  it('should apply custom formatLinkUrl when provided', () => {
    mockUseImageRepository.mockReturnValue([mockPublicImageRepository, true, null]);
    const customFormatter = (url: string) => `https://custom.registry/${url}`;
    ...
    renderWithQueryClient(
      <ImageUrlDisplay
        ...
        namespace={testNamespace}
        componentName={testComponentName}
        formatLinkUrl={customFormatter}
      />,
    );

    const link = screen.getByRole('link');
    expect(link).toHaveAttribute('href', `https://custom.registry/${testImageUrl}`);
  });
  ...
  it('should apply custom linkStyle when provided', () => {
    mockUseImageRepository.mockReturnValue([mockPublicImageRepository, true, null]);
    const customStyle = { userSelect: 'auto' as const };
    ...
    renderWithQueryClient(
      <ImageUrlDisplay
        ...
        namespace={testNamespace}
        componentName={testComponentName}
        linkStyle={customStyle}
      />,
    );

    const link = screen.getByRole('link');
    expect(link).toHaveStyle({ userSelect: 'auto' });
  });
});
