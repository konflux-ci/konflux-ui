...
- No manual `addEventListener`/`removeEventListener` -- use `useEventListener` from `~/shared/hooks/useEventListener` for browser event listeners. Exceptions: `ResizeObserver`/`IntersectionObserver` callbacks, `useSyncExternalStore` subscriptions, and listeners on dynamically created or non-React-managed DOM nodes. See `docs/best-practices.md` for details.
